...
if __name__ == "__main__":
    ...
    # point = Point(sensor[0, :, 0])
    # fig.add_trace(go.Scatter(x=[point.xy[0][0]], y=[point.xy[1][0]], mode='markers'))
    #
    # radius = list(point.buffer(5).exterior.coords)
    # fig.add_trace(go.Scatter(x=[x[0] for x in radius], y=[x[1] for x in radius], mode='lines'))

    # gater = []
    for i in range(time_steps):
        # graphs = []
        # for j in range(sensor.shape[2]):
        point = Point(sensor[i, :, 0])
        radius = list(point.buffer(5).exterior.coords)
        fig.add_trace(go.Scatter(x=[x[0] for x in radius], y=[x[1] for x in radius], mode='lines'))

        # fig.add_trace(go.Scatter(x=[point.xy[0][0]], y=[point.xy[1][0]], mode='markers'))

        # buff = Point(sensor[i, :, j])
        # graphs.append(buff)
        # buffer = buff.buffer(0.2).exterior.coords
        # fig.add_trace(go.Scatter(x=[x[1] for x in buffer], y=[x[1] for x in buffer], mode='markers'))
