...
def random_walk(num_sensors=1, time=10, reading=10):
    ...
    sensor_data = sensor_data + (np.random.rand(sensor_data.shape[0], sensor_data.shape[1]) / 2.1)
    return sensor_data
...
def track_random_walk(actual_track):
    ...
    final_track = track_ + (np.random.rand(track_.shape[0]) / 2.1)
...
if __name__ == "__main__":
    ...
    for i in range(time):
        if i > 0:
            # previous_diff = np.round(sensor[i, :] - sensor[i-1, :])
            previous_diff = sensor[i, :] - sensor[i-1, :]

        else:
            # previous_diff = np.round(sensor[i, :] - 0)
            previous_diff = sensor[i, :] - 0

        total_dist += previous_diff
